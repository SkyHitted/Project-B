...
namespace projectBGroep3
{
    ...
    {
        public static void Reserveren(int id) // reserveren functie
        {
            string s = "";
            string title =
@"██████╗░███████╗░██████╗███████╗██████╗░██╗░░░██╗███████╗██████╗░███████╗███╗░░██╗
...
╚═╝░░╚═╝╚══════╝╚═════╝░╚══════╝╚═╝░░╚═╝░░░╚═╝░░░╚══════╝╚═╝░░╚═╝╚══════╝╚═╝░░╚══╝" + "\n";
            ...
            var jsonString = File.ReadAllText(fileNameReserveren); // json openslots
            var existReserveren = JsonSerializer.Deserialize<NewDay>(jsonString)!;
            string fileNameConfirm = "ConfirmReserveren.json";
            var jsonConfirmString = File.ReadAllText(fileNameConfirm);// json bevestiging
            var existConfirm = JsonSerializer.Deserialize<NewReservering>(jsonConfirmString)!;
            s = "Voer in de naam van uw reservering:\n";
            string annuleer = "\n(Voer in \"Afbreken\" om de reservering te stoppen, zonder aanhalingstekens.)"; // annuleer melding
            string melding = "";
            for (bool reserveren_Loop = true; reserveren_Loop;) // invoer naam reservering
            {
                Console.Clear();
                Console.WriteLine(title);
                projectBGroep3.Program.getMelding(melding); // foutmelding naam
                Console.WriteLine(s + annuleer);
                string naamVanReserveren = Console.ReadLine();
                if (naamVanReserveren == "Afbreken") // stopt reservering
                {
                    return;
                }
                ...
                {
                    Console.Clear();
                    string allPossibleDates = ReserverenClass.GetDates(); // lijst van 14 dagen vooruit reserveren
                    melding = "";
                    s = $"Op welke datum wilt u reserveren? U kan 2 weken vooruit reserveren.\nVoer in uw gekozen datum van de lijst hieronder:\n\n{allPossibleDates}";
                    for (bool dateLoop = true; dateLoop;) // invoer datum reservering
                    {
                        Console.WriteLine(title);
                        projectBGroep3.Program.getMelding(melding); // foutmelding datum als er is
                        Console.WriteLine(s + annuleer);
                        ...
                        if (DateTime.TryParse(stringOfDateTime, out var chosenDate)) // c# bool verander string naar DateTime
                        {
                            ...
                            if (chosenDate >= todayDate && chosenDate <= DateTime.Today.AddDays(14) && (chosenDate.DayOfWeek == DayOfWeek.Wednesday || chosenDate.DayOfWeek == DayOfWeek.Thursday || chosenDate.DayOfWeek == DayOfWeek.Friday))
                            {
                                Console.Clear();
                                melding = "";
                                s = "In welk tijdslot wilt u graag reserveren?\nEen tijdslot is 2 uren, van 10:00 tot en met 22:00.\n\nVoer in: 10, 12, 14, 16, 18, 20 of 22.";
                                for (bool timeLoop = true; timeLoop;) // invoer tijdslot reserveren
                                {
                                    Console.WriteLine(title);
                                    ...
                                    if (int.TryParse(stringOfChosenTime, out int chosenTimeSlot))//(int.TryParse(Console.ReadLine(), out int chosenTimeSlot))
                                    {
                                        ...
                                        if (timeSlots.Contains(chosenTimeSlot)) // invoer gebruiker in lijst True
                                        {
                                            Console.Clear();
                                            int indexOfChosenDate = 0;
                                            for (int i = 0; i < existReserveren.Reserveren.Count; i++) // loop json om index van datum te krijgen
                                            {
                                                if (existReserveren.Reserveren[i].deDatum == chosenDate.Date.ToString())
                                                {
                                                    indexOfChosenDate = i; // index
                                                }
                                            }
                                            melding = "";
                                            var gastenOpChosenTime = existReserveren.Reserveren[indexOfChosenDate].allTime[0].openSlotten[chosenTimeSlot]; // hoeveel gasten open op gekozen datum en tijdslot
                                            if (gastenOpChosenTime <= 0) // geen zitplaatsen open
                                            {
                                                melding = $"Er is geen beschikbare zitplaats op tijdslot: {chosenTimeSlot}.\n";
                                                continue;
                                            }
                                            else if (gastenOpChosenTime == 1) // nog 1 zitplaats
                                            {
                                            ...
                                            for (bool gastLoop = true; gastLoop;) // invoer aantal gasten reserveren
                                            {
                                                Console.WriteLine(title);
                                                projectBGroep3.Program.getMelding(melding);
                                                Console.WriteLine(s + annuleer);
                                                ...
                                                {
                                                    if (gastenReserveren <= 0) // minder dan minimaal
                                                    {
                                                        Console.Clear();
                                                        melding = "U kan niet reserveren voor minder dan 1 persoon.\n";
                                                        continue;
                                                    }
                                                    else if (gastenOpChosenTime - gastenReserveren < 0)
                                                    {
                                                        Console.Clear();
                                                        melding = "U kan niet meer dan de aantal beschikbare zitplaatsen invoeren.\n";
                                                        continue;
                                                    }
                                                    else if (gastenOpChosenTime == 1 && gastenReserveren > 1) // 1 zitplaats open en invoer is groter dan 1
                                                    {
                                                        Console.Clear();
                                                        melding = "U heeft meer dan 1 ingevoerd.\n";
                                                        continue;
                                                    }
                                                    else if (gastenReserveren > 8) // meer dan maximaal
                                                    {
                                                        Console.Clear();
                                                    ...
                                                    {
                                                        Console.Clear();
                                                        Dictionary<int, string> tmpSmallDict = new Dictionary<int, string>() // dict c#
                                                                {
                                                        ...
                                                        melding = "";
                                                        for (bool bevestigLoop = true; bevestigLoop;) // invoer bevestiging reserveren
                                                        {
                                                            Console.WriteLine(title);
                                                            projectBGroep3.Program.getMelding(melding);
                                                            ...
                                                            {
                                                                Console.Clear();
                                                                s = $"Dit is uw bevestigingscode voor deze reservering:\n{bevestigingCodeMade}\n\nDruk op Enter om verder te gaan."; //Als u deze reservering wilt annuleren voer dan \"annuleer\" in zonder aanhalingstekens.
                                                                Console.WriteLine(s);
                                                                Console.ReadKey();
                                                                existReserveren.Reserveren[indexOfReserveren].allTime[0].openSlotten[chosenTimeSlot] -= gastenReserveren; // haalt aantal gasten van json
                                                                existReserveren.Reserveren[indexOfReserveren].allTime[0].reserveert[chosenTimeSlot] += 1; // json gereserveert +1
                                                ...
                                                {
                                                    Console.Clear();
                                                    melding = "U heeft een geen nummer ingevoerd.\n";
                                                    continue;
                                                }
                                            }
                                        }
                                        ...
                                        {
                                            Console.Clear();
                                            melding = "U heeft een ongeldig tijd ingevoerd.\n";
                                            continue;
                                        }
                                    }
                                    else
                            ...
                            {
                                Console.Clear();
                                melding = "U heeft een verkeerde datum ingevoerd.\n";
                                continue;
                            }
                        }
                        else
                        {
                            Console.Clear();
                            melding = "U heeft een geen datum ingevoerd.\n";
                            continue;
                        }
                    }
                }
        ...
        public static void HigherReserveren(int id) // functie reserveren voor admin en werknemer
        {
            string s = "";
            string title =
@"██████╗░███████╗░██████╗███████╗██████╗░██╗░░░██╗███████╗██████╗░███████╗███╗░░██╗
...
██║░░██║███████╗██████╔╝███████╗██║░░██║░░╚██╔╝░░███████╗██║░░██║███████╗██║░╚███║
╚═╝░░╚═╝╚══════╝╚═════╝░╚══════╝╚═╝░░╚═╝░░░╚═╝░░░╚══════╝╚═╝░░╚═╝╚══════╝╚═╝░░╚══╝" + "\n";
            for (bool screenHigherReserverenLoop = true; screenHigherReserverenLoop;)
            {
                s = "";
            int screenAdminReserveren = Options.listOfChoice(true, title + s, "Reserveren\n", "Zitplaatsen toevoegen of verwijderen\n", "Ga terug\n");
                ...
                else if (screenAdminReserveren == 1) // zitplaatsen toevoegen/verwijderen
                {
                    string fileNameReserveren = "OpenSlotsReserveren.json";
                                ...
                                for (bool timeLoop = true; timeLoop;)
                                {
                                    ...
                                    if (int.TryParse(stringOfChosenTime, out int chosenTimeSlot))//(int.TryParse(Console.ReadLine(), out int chosenTimeSlot))
                                    {
                                        ...
                                        if (timeSlots.Contains(chosenTimeSlot))
                                        {
                                            ...
                                            {
                                                s = $"Er is {gastenOpChosenTime} beschikbare zitplaats op dit tijdslot.\nVoer in hoeveel zitplaatsen u wilt toevoegen of verwijderen (verwijderen door middel van -).\n";
                                            }
                                            else
                                            {
                                                s = $"Er zijn {gastenOpChosenTime} beschikbare zitplaatsen op dit tijdslot.\nVoer in hoeveel zitplaatsen u wilt toevoegen of verwijderen (verwijderen door middel van -).\n";
